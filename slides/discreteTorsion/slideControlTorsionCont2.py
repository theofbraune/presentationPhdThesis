...
class slideControllingTorsionCurvatureCont2(Slide):
    def construct(self):
        ...
        self.add(textTitle)
        textConclusion = Tex(r"Elegant structure preserving discretization of torsion of connections on surfaces", font_size=24).next_to(textTitle, 2*DOWN, aligned_edge=LEFT, buff=0.4)
        self.play(FadeIn(textConclusion))
        self.next_slide()
        textNext = Tex(r"Enriches the design space for methods that control curvature in geometry processing ",font_size=24).next_to(textConclusion, 2*DOWN, aligned_edge=LEFT, buff=0.3)
        self.play(FadeIn(textNext))
        self.next_slide()
        textHowever = Tex(r"However, heavily dependent on the fact that we consider the torsion form on the tangent bundle of a surface  ", font_size=24).next_to(textNext, 2*DOWN, aligned_edge=LEFT, buff=0.3)
        self.play(FadeIn(textHowever))
        self.next_slide()
        textNoGeneralization = Tex(r"How can we generalize the structure preserving discretization of arbitrary bundle valued forms on arbitrary manifolds?", font_size=24).next_to(textHowever, 2* DOWN, aligned_edge=LEFT, buff=0.3)
